...
var currentCat = "all";
var currentUser = null; // We will check auth state on load
...
function showTab(tab, el) {
    ...
    else document.querySelectorAll(".nav-link[data-tab='" + tab + "']").forEach(function(n) { n.classList.add("active"); });
...
firebase.auth().onAuthStateChanged(function(firebaseUser) {
    if (firebaseUser) {
        // User is signed in. Let's get their data from our database.
        db.ref('customers/' + firebaseUser.uid).once('value').then(function(snap) {
            if (snap.exists()) {
                setCurrentUser(firebaseUser.uid, snap.val());
            } else {
                // This shouldn't happen often, but if it does, create a profile.
...
function sendEmailLink(e) {
    ...
    var actionCodeSettings = {
        url: window.location.origin, // Use the current website URL
...
function signInWithGoogle() {
    ...
    firebase.auth().signInWithPopup(provider)
        .then(function(result) {
            processUserLogin(result.user, result.additionalUserInfo.isNewUser);
        }).catch(function(error) { showToast('Google Sign-in failed: ' + error.message); });
}
...
function processUserLogin(firebaseUser, isNew) {
    ...
    if (isNew) {
        ...
        db.ref('customers/' + userId).set(userData).then(function() {
            ...
            showTab('profile'); // Send new users to profile to fill details
        });
    } else {
        // Data will be fetched by onAuthStateChanged listener, just redirect.
...
// --- Products and Cart Logic (Mostly Unchanged) ---
db.ref("products").on("value", function(snap) {
    var data = snap.val();
    document.getElementById("loading").classList.add("hidden");
    if (!data) {
        document.getElementById("products-grid").innerHTML = '<div class="empty-box" style="grid-column:1/-1"><h3>No products yet</h3></div>';
        allProducts = {};
    } else {
        allProducts = data;
        renderProducts();
...
function renderProducts() {
    ...
    if (filtered.length === 0) {
        grid.innerHTML = '<div class="empty-box" style="grid-column:1/-1"><h3>No products in this category</h3></div>';
        return;
    }
...
function renderCart() {
    ...
    var empty = document.getElementById("cart-empty"), filled = document.getElementById("cart-filled");
    var items = document.getElementById("cart-items"), lines = document.getElementById("summary-lines"), totalEl = document.getElementById("summary-total");
    if (cart.length === 0) { empty.classList.remove("hidden"); filled.classList.add("hidden"); return; }
...
// --- Order Placement, Tracking, and Cancelling (Mostly Unchanged) ---
function placeOrder() {
    ...
    db.ref("products").once("value").then(function(snap) {
        ...
        var promises = cart.map(function(c) { return db.ref("products/" + c.id + "/onlineStock").transaction(function(cur) { return Math.max(0, (cur || 0) - c.qty); }); });
        Promise.all(promises).then(function() {
            var orderId = "ORD-" + Date.now().toString().slice(-8);
            var orderData = { customerName: name, phone: phone, address: address, items: items, total: total, courier: courier, receipt: receiptData, status: "Pending", customerId: currentUser ? currentUser.id : "", customerEmail: currentUser ? currentUser.email : "", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
            db.ref("orders/" + orderId).set(orderData).then(function() {
                ...
                document.getElementById("suc-btn-orders").style.display = currentUser ? "" : "none";
                document.getElementById("suc-btn-track").style.display = currentUser ? "none" : "";
...
function loadMyOrders() {
    if (!currentUser) { showTab("auth"); return; }
    var container = document.getElementById("myorders-list"); container.innerHTML = '<div class="loading"><div class="spinner"></div><p>Loading orders...</p></div>';
    db.ref("orders").orderByChild("customerId").equalTo(currentUser.id).once("value").then(function(snap) {
        ...
        container.innerHTML = orders.map(function(e) {
            var id = e[0], o = e[1], sc = statusBadgeClass(o.status), itemsList = o.items.map(function(i) { return (i.emoji || "🥩") + " " + i.name + " × " + i.qty; }).join(" • ");
...
function confirmCancelOrder() {
    ...
    db.ref("orders/" + cancelTarget).once("value").then(function(snap) {
        var order = snap.val(); if (!order || order.status !== "Pending") { showToast("This order can no longer be cancelled."); closeCancelModal(); return; }
        var promises = order.items.map(function(item) { return db.ref("products/" + item.productId + "/onlineStock").transaction(function(cur) { return (cur || 0) + item.qty; }); });
        Promise.all(promises).then(function() {
            return db.ref("orders/" + cancelTarget).update({ status: "Cancelled", updatedAt: new Date().toISOString() });
        }).then(function() { showToast("Order " + cancelTarget + " has been cancelled."); closeCancelModal(); loadMyOrders(); });
    });
}
function trackOrder() { /* Unchanged */ }

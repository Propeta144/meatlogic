...
function aTab(name, el) {
    ...
    var titles = { dashboard:"Dashboard", stock:"Stock Management", orders:"Orders", products:"Products", customers:"Customers" };
...
function refreshDash() {
    ...
    var todayOrds = ordArr.filter(function(e) { return new Date(e[1].createdAt).toDateString() === today; });
    var pending = ordArr.filter(function(e) { return e[1].status === "Pending"; });
    var revenue = todayOrds.filter(function(e) { return e[1].status !== "Rejected" && e[1].status !== "Cancelled"; })
        .reduce(function(s, e) { return s + e[1].total; }, 0);
    var userCount = Object.keys(adminCustomers).length;
    ...
    if (todayOrds.length === 0) { recent.innerHTML = '<p class="muted">No orders today.</p>'; }
        ...
        recent.innerHTML = todayOrds.slice(0, 5).map(function(e) {
            var id = e[0], o = e[1], sc = statusClass(o.status);
    ...
    var low = prodArr.filter(function(e) { return (e[1].onlineStock || 0) <= 5; });
    if (low.length === 0) { lowEl.innerHTML = '<p class="muted">All well stocked.</p>'; }
    else {
        lowEl.innerHTML = low.map(function(e) {
            var p = e[1], stk = p.onlineStock || 0;
            var cls = stk === 0 ? "stk-out" : "stk-low";
...
function initProducts() { loadDefaultProducts().then(function() { showToast("Default products loaded!"); }); }

/* ---- STOCK ---- */
function renderStockGrid() {
    var grid = document.getElementById("stock-grid");
    var entries = Object.entries(adminProducts);
    if (entries.length === 0) { grid.innerHTML = '<p class="muted">No products yet.</p>'; return; }
    grid.innerHTML = entries.map(function(e) {
        var id = e[0], p = e[1];
        return '<div class="sk-card"><div class="sk-top"><div class="sk-emoji">' + (p.emoji || "🥩") + '</div><div><div class="sk-name">' + p.name + '</div><div class="sk-meta">' + p.category + ' • ' + formatCurrency(p.price) + '</div></div></div><div class="sk-input-row"><label>Stock:</label><input type="number" class="sk-input" id="sk-' + id + '" value="' + (p.onlineStock || 0) + '" min="0" max="999"></div></div>';
    }).join("");
}
function saveStock() {
    var updates = {};
    Object.keys(adminProducts).forEach(function(id) {
        var inp = document.getElementById("sk-" + id);
        if (inp) updates["products/" + id + "/onlineStock"] = parseInt(inp.value) || 0;
    });
    db.ref().update(updates).then(function() { showToast("Stock updated!"); });
}

/* ---- ORDERS ---- */
function renderOrders() {
    var list = document.getElementById("orders-list");
    var entries = Object.entries(adminOrders).sort(function(a, b) { return new Date(b[1].createdAt) - new Date(a[1].createdAt); });
    if (orderFilter !== "all") entries = entries.filter(function(e) { return e[1].status === orderFilter; });
    ...
    list.innerHTML = entries.map(function(entry) {
        ...
        return '<div class="ao-card">' +
            '<div class="ao-header" onclick="toggleDet(\'' + id + '\')">' +
                '<div class="ao-info"><h4>' + id + ' — ' + o.customerName + '</h4><p>' + formatDate(o.createdAt) + ' • ' + o.courier + ' • ' + formatCurrency(o.total) + '</p></div>' +
            ...
            '<div class="ao-detail" id="det-' + id + '">' +
                '<div class="det-grid">' +
                    ...
                    '</div></div>' +
                '</div>' +
                '<div class="det-items"><h5>Items</h5>' +
                    o.items.map(function(i) { return '<div class="di-row"><span>' + (i.emoji || "🥩") + ' ' + i.name + ' × ' + i.qty + ' ' + i.unit + '</span><span>' + formatCurrency(i.lineTotal) + '</span></div>'; }).join("") +
...
function getActions(id, o) {
    switch (o.status) {
        case "Pending": return '<button class="btn btn-success btn-sm" onclick="setStatus(\'' + id + '\',\'Approved\')">✅ Approve</button><button class="btn btn-sm" style="background:var(--red-500);color:#fff;" onclick="setStatus(\'' + id + '\',\'Rejected\')">❌ Reject</button>';
        case "Approved": return '<div style="width:100%"><small style="color:var(--text2)">' + (o.courier === "Lalamove" ? "Rider Name & Contact" : "LBC Tracking Number") + '</small><div class="trk-group"><input type="text" class="trk-input" id="trk-' + id + '" placeholder="' + (o.courier === "Lalamove" ? "e.g. Mark - 0918 XXX" : "e.g. LBC-123456") + '" value="' + (o.riderInfo || o.trackingInfo || "") + '"><button class="btn btn-sm" style="background:var(--orange-500);color:#fff;" onclick="shipOrder(\'' + id + '\')">🚚 Ship</button></div></div>';
        case "Out for Delivery": return '<button class="btn btn-success btn-sm" onclick="setStatus(\'' + id + '\',\'Completed\')">✅ Completed</button><span style="font-size:.83rem;color:var(--text2)">' + (o.courier === "Lalamove" ? "🏍️ " + (o.riderInfo || "") : "📦 " + (o.trackingInfo || "")) + '</span>';
    ...
    if (status === "Rejected") {
        ...
        if (o && o.status === "Pending" && o.items) {
            o.items.forEach(function(item) {
                db.ref("products/" + item.productId + "/onlineStock").transaction(function(cur) { return (cur || 0) + item.qty; });
            });
        }
    }
    db.ref("orders/" + id).update(updates).then(function() { showToast(id + " → " + status); });
}

function shipOrder(id) {
    var inp = document.getElementById("trk-" + id);
    ...
    db.ref("orders/" + id).update(updates).then(function() { showToast(id + " → Out for Delivery!"); });
}
...
function renderProdTable() {
    ...
    var entries = Object.entries(adminProducts);
    if (entries.length === 0) { tbody.innerHTML = '<tr><td colspan="6" class="muted">No products.</td></tr>'; return; }
    tbody.innerHTML = entries.map(function(e) {
        var id = e[0], p = e[1];
        return '<tr><td><div class="tp-cell"><div class="tp-emoji">' + (p.emoji || "🥩") + '</div><span>' + p.name + '</span></div></td><td><span class="p-cat-tag">' + p.category + '</span></td><td><strong>' + formatCurrency(p.price) + '</strong></td><td>' + p.unit + '</td><td><strong>' + (p.onlineStock || 0) + '</strong></td><td><div class="t-actions"><button class="t-btn t-btn-edit" onclick="editProd(\'' + id + '\')">Edit</button><button class="t-btn t-btn-del" onclick="delProd(\'' + id + '\')">Delete</button></div></td></tr>';
    }).join("");
}
...
function editProd(id) {
    var p = adminProducts[id]; if (!p) return;
...
function saveProd(e) {
    ...
    db.ref("products/" + id).set(data).then(function() { closeProdModal(); showToast("Product saved!"); });
}
function delProd(id) {
    if (!confirm("Delete this product?")) return;
    db.ref("products/" + id).remove().then(function() { showToast("Product deleted."); });
}
...
function renderCustomers() {
    ...
    if (entries.length === 0) { tbody.innerHTML = '<tr><td colspan="5" class="muted">No registered customers yet.</td></tr>'; return; }
    tbody.innerHTML = entries.map(function(e) {
        var c = e[1];
        return '<tr><td><strong>' + (c.fullName || c.firstName + " " + c.lastName) + '</strong></td><td>' + c.email + '</td><td>' + (c.phone || "-") + '</td><td>' + (c.address || "-") + '</td><td>' + formatDate(c.createdAt) + '</td></tr>';
    }).join("");
}